...
void main() {
    ...
    // unpack normal
    vec3 nts = texture(normalMap, tc).xyz * 2.0 - 1.0;
    nn = normalize(tbn * nts);

    vec4 c;
    if (tex == 0)
        c = texture(diffuseY, tc);
    else if (tex == 1)
        c = texture(diffuseR, tc);
    else if (tex == 2)
        c = texture(diffuseG, tc);
    else if (tex == 3)
        c = texture(diffuseB, tc);
    else 
        c = texture(diffuseBl, tc);

    vec4 r = texture(rust, tc);

    float rand = perlinNoise(pw*2);
    ...
    if (intensity > 0.0 && rand < 0.4) {
            vec3 h = normalize(ldn + eye);
            float specInt = max(0.0, dot(nn,h));
            vec4 specMap = texture(specularMap, tc); 

            specular = specMap * pow(specInt, 80);
    }

    color =  c* 0.50 + intensity * c + specular;
}
